      ...
      <el-form
        ...
        <el-tabs
          ...
          <el-tab-pane label="商品描述" name="4">
            ...
            <el-button type="primary" class="btnAdd" @click="addGoods">修改商品</el-button>
          </el-tab-pane>
        </el-tabs>
      </el-form>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleRemove(file) {
        ...
        this.goodsObj.pics.forEach((item, index) => {
          let temporary = 'http://127.0.0.1:8888' + item.pics_sma
          if (temporary === file.url) {
      ...
      addGoods() {
        this.$refs.ruleForm.validate(async valid=>{
          ...
          const { data : res } = await this.$axios.put(`goods/${this.goodsObj.goods_id}`,this.goodsObj)
    ...
    async created() {
      ...
      let { data : res } = await this.$axios.get(`goods/${this.$route.query.id}`)
      console.log(res)
      this.goodsObj = res.data
      let arr = [];
      this.goodsObj.goods_cat = this.goodsObj.goods_cat.split(",");
      this.goodsObj.goods_cat.map((item) => {
        item = parseInt(item);
        arr.push(item);
      });
      this.goodsObj.goods_cat = arr;
